...
const send_psreset_email = async (e) => {
  ...
  try {
    ...
    if (res.data.status == "success") {
      ...
      alert("Unknown error occured during request!");
      e.target.disabled = false;
    }
...
<template>
  <div class="border border-transparent">
    <div class="w-full h-screen -mt-10 relative grid content-center">
      <div
        class="max-w-sm sm:max-w-md md:max-w-lg !w-full mx-auto text-center py-10 rounded-xl px-2 shadow-md"
      >
        ...
        <button
          class="mt-3 group border max-w-sm w-full py-2 rounded-lg ring-2 disabled:ring-0"
          @click="send_psreset_email($event)"
          id="resend_button"
        >
          <i
            class="fas fa-spinner !hidden group-disabled:!inline-block animate-spin"
          ></i>
          send Reset Link
        </button>
      </div>
