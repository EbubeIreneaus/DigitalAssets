      ...
      <div class="max-w-sm w-full border py-10 mx-auto text-white/90 px-7">
        ...
        <div class="py-5">
          <form @submit.prevent="confirmUser($event)">
            <p
              class="py-1.5 bg-red-600/50 px-7 my-2 text-red-500/80 rounded"
              v-if="error"
            >
              <i class="fa fa-warning"></i>&nbsp;
              {{ error_msg }}
            </p>
            <label>Enter your password<span class="text-red-500">*</span></label
            ><br />
            <input
              type="password"
              autofocus
            ...
            <button
              type="submit"
              id="btn"
              class="block w-full py-3 bg-orange-600 hover:bg-orange-500 rounded-md"
            >
              Confirm
            </button>
          </form>
...
const confirmUser = async (e) => {
  ...
  const {
    ...
    method: "get",
    query: {
      username: account.value.profile.user.username,
